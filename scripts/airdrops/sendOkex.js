...
async function main() {
    ...
    const works = await hre.network.provider.request({
    ...
    const signer = ethers.provider.getSigner("0x02491D37984764d39b99e4077649dcD349221a62")
/** 
    for (let i = 0; i < allItemObj.length; i++) {

        const sendrecipients = await dao.connect(signer).mintItems(recipients[i], allItemObj[i], quantities)
...
**/
    ...
    for (let j = 0; j < champs.length; j++) {
        const sendToChamps = await dao.connect(signer).mintItems(champs[j], fullsets, quantity)
